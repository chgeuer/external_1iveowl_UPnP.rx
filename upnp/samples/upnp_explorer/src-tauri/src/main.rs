...
fn main() {
    tauri::Builder::default()
        ...
        .menu(|handle| {
            let quit = MenuItem::with_id(handle, "quit", "Quit UPnP Explorer", true, Some("CmdOrCtrl+Q"))?;
            let app_menu = Submenu::with_items(handle, "UPnP Explorer", true, &[&quit])?;
...
fn connect_channel() -> Option<ChannelStream> {
    #[cfg(unix)]
    {
        ...
        let socket_path = std::env::temp_dir().join("tauri_heartbeat_upnp_explorer.sock");
    ...
    #[cfg(windows)]
    {
        ...
        let port_file = std::env::temp_dir().join("tauri_heartbeat_upnp_explorer.port");
